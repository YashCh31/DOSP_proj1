%% 
%% This is a wrapper around the webs_socket_server.  It's based
%% on 'webs_http' but handles the WebSocket protocol.
...
stop() ->
    webs_socket_server:stop(?MODULE).

stop(Name) ->
    webs_socket_server:stop(Name).
...
start(Options) ->
    webs_socket_server:start(parse_options(Options)).
